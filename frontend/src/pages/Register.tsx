...
import { Mail, Lock, User, Eye, EyeOff, Gamepad2, Check } from 'lucide-react'
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4">
      ...
      >
        ...
        <div className="text-center">
          ...
          <p className="text-white/60">Join the future of gaming today</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {formData.password && (
                <div className="mt-2 space-y-1">
                  <div className="flex items-center space-x-2 text-xs">
                    <div className={`w-2 h-2 rounded-full ${passwordValidation.minLength ? 'bg-green-400' : 'bg-white/20'}`} />
                    <span className={passwordValidation.minLength ? 'text-green-400' : 'text-white/40'}>
                      At least 8 characters
                    </span>
                  </div>
                  <div className="flex items-center space-x-2 text-xs">
                    <div className={`w-2 h-2 rounded-full ${passwordValidation.hasUpper ? 'bg-green-400' : 'bg-white/20'}`} />
                    <span className={passwordValidation.hasUpper ? 'text-green-400' : 'text-white/40'}>
                      One uppercase letter
                    </span>
                  </div>
                  <div className="flex items-center space-x-2 text-xs">
                    <div className={`w-2 h-2 rounded-full ${passwordValidation.hasLower ? 'bg-green-400' : 'bg-white/20'}`} />
                    <span className={passwordValidation.hasLower ? 'text-green-400' : 'text-white/40'}>
                      One lowercase letter
                    </span>
                  </div>
                  <div className="flex items-center space-x-2 text-xs">
                    <div className={`w-2 h-2 rounded-full ${passwordValidation.hasNumber ? 'bg-green-400' : 'bg-white/20'}`} />
                    <span className={passwordValidation.hasNumber ? 'text-green-400' : 'text-white/40'}>
                      One number
                    </span>
                  </div>
                </div>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm text-white/60">
                I agree to the{' '}
                <Link to="/terms" className="text-neon-blue hover:text-neon-purple transition-colors duration-200">
                  Terms of Service
                </Link>{' '}
                and{' '}
                <Link to="/privacy" className="text-neon-blue hover:text-neon-purple transition-colors duration-200">
                  Privacy Policy
                </Link>
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading || !passwordValidation.isValid || formData.password !== formData.confirmPassword || !agreedToTerms}
